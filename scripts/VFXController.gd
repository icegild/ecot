...
typealias VFXParams = Dictionary
...
static func _make_particles(
	...
	particles.set_meta("auto_destroy", true)
...
static func _add_to_scene(particles: GPUParticles2D) -> void:
	...
	var lifetime: float = 2.0  # generous timeout
	if particles.process_material is ParticleProcessMaterial:
		...
		lifetime = mat.lifetime * (1.0 + mat.lifetime_randomness) + 0.5
