...
var patrol_flip_cooldown: float = 0.0
var chase_jump_cooldown: float = 0.0
...
func _physics_process(delta: float) -> void:
	...
	if patrol_flip_cooldown > 0.0:
		patrol_flip_cooldown -= delta
	if chase_jump_cooldown > 0.0:
		chase_jump_cooldown -= delta
...
func _state_patrol(delta: float) -> void:
	# Smooth acceleration toward patrol speed
	var target_vx: float = patrol_direction * move_speed * 0.4
	velocity.x = move_toward(velocity.x, target_vx, move_speed * 3.0 * delta)
	facing_direction = patrol_direction
	patrol_timer += delta
	if patrol_timer > 3.0:
		current_state = State.IDLE
		patrol_timer = 0.0
	# Only reverse direction if cooldown has elapsed (prevents rapid flip)
	if patrol_flip_cooldown <= 0.0:
		if is_on_wall():
			patrol_direction *= -1.0
			patrol_flip_cooldown = 0.4
...
func _state_chase(delta: float) -> void:
	...
	# Smooth acceleration toward chase speed
	var target_vx: float = dir * move_speed
	velocity.x = move_toward(velocity.x, target_vx, move_speed * 8.0 * delta)
	# Only jump when on floor, player is significantly above, and cooldown allows
	if is_on_floor() and chase_jump_cooldown <= 0.0 and player_ref.global_position.y < global_position.y - 60.0:
		velocity.y = jump_force
		chase_jump_cooldown = 0.8
